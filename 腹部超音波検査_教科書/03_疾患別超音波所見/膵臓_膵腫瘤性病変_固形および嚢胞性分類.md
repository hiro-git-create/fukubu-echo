...
```mermaid
graph TD
    PancreasMass["膵腫瘤性病変 (Pancreatic Mass)"]
    
    PancreasMass --> Solid["1. 固形腫瘤性病変 (Solid)"]
    PancreasMass --> Cystic["2. 嚢胞性腫瘤性病変 (Cystic)"]

    subgraph SubSolid ["【固形腫瘤 (Solid Lesions)】"]
        direction TB
        Solid --> PDAC["通常型膵がん (PDAC)<br>浸潤性管がん (90%)"]
        Solid --> NET["膵神経内分泌腫瘍<br>(PanNET / PanNEN)"]
        Solid --> SPN["Solid Pseudopapillary<br>Neoplasm (SPN)"]
        Solid --> AIP["自己免疫性膵炎 (AIP)<br>腫瘤形成性膵炎"]
    end

    subgraph SubCystic ["【嚢胞性腫瘤 (Cystic Lesions)】"]
        direction TB
        Cystic --> Tumorous["腫瘍性嚢胞"]
        Cystic --> NonTumorous["非腫瘍性嚢胞"]
        
        Tumorous --> IPMN["IPMN (膵管内乳頭粘液性)"]
        Tumorous --> MCN["MCN (粘液性嚢胞腫瘍)"]
        Tumorous --> SCN["SCN (漿液性嚢胞腫瘍)"]
        
        NonTumorous --> Pseudocyst["仮性嚢胞 (Pseudocyst)"]
        NonTumorous --> Retention["停留性嚢胞 (Retention)"]
    end
```
