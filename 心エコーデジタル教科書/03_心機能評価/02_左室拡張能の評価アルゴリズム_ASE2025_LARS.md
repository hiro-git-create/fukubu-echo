...
| 指標 | カットオフ値 | 補足 | 臨床的意義 |
| :--- | :---: | :--- | :--- |
| **e'（僧帽弁輪弛緩速度）** | **平均 ≤ 6.5 cm/s** | 中隔 ≤ 6 / 側壁 ≤ 7 | 心筋弛緩能低下の指標 |
| **E/e' 比（平均）** | **> 14.0** | 中隔 > 15 / 側壁 > 13 | 左室充満圧（LAP）上昇の指標 |
| **★ LARS（左房リザーバーストレイン）** | **≤ 18.0 %** | ASE 2025 新導入 | **早期の左房機能不全・LAP上昇の最高感度指標** |
| **E/A 比** | **≤ 0.8 または ≥ 2.0** | — | 僧帽弁流入血流パターン異常 |
| **LAVi（左房容量係数）** | **> 34.0 mL/m²** | — | 慢性的な左房圧負荷の指標 |
| **PV S/D（肺静脈血流速度比）** | **≤ 0.67** | — | 収縮期/拡張期血流比の低下（LAP上昇） |
| **TRV（三尖弁逆流速度）** | 補助指標 | 診断ステップから除外 | 充満圧推定の補助として使用 |
...
| グレード | 状態（病態） | LAP | 主な血流パターン |
| :---: | :--- | :---: | :--- |
| **Normal** | 正常な拡張機能 | 正常 | E/A = 0.8〜2.0、LARS > 18% |
| **Grade I** | 弛緩能不全（軽度） | 正常 | E/A ≤ 0.8、減速時間（DT）延長 |
| **Grade II** | 偽正常化パターン（中等度） | **上昇** | E/A = 0.8〜2.0 ＋ LARS ≤ 18% 等の充満圧上昇所見あり |
| **Grade III** | 拘束型パターン（重度） | **著明上昇** | E/A ≥ 2.0、DT短縮（< 160 ms） |
