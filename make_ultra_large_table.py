...
# Ultra Large Text Portal HTN Table (Height: 750px)
def make_ultra_large_portal_table():
    w, h = 1350, 720
    img = Image.new("RGB", (w, h), (255, 255, 255))
    draw = ImageDraw.Draw(img)
    
    # Top Title Box
    draw.rectangle([(20, 15), (1330, 75)], fill="#065f46")
    draw.text((675, 45), "門脈高血圧症 (Portal Hypertension) 超音波血流動態評価表", fill="#ffffff", font=font_title, anchor="mm")
    ...
    y = 90
    draw.rectangle([(25, y), (1325, y + 55)], fill="#059669")
    ...
    for i, (item, norm, abn, desc) in enumerate(data):
        ...
        # Col 1
        draw.rectangle([(25, cur_y), (275, cur_y + row_h)], outline="#cbd5e1", width=2)
        draw.text((150, cur_y + row_h // 2), item, fill="#065f46", font=font_large, anchor="mm")
        
        # Col 2
        draw.rectangle([(275, cur_y), (555, cur_y + row_h)], outline="#cbd5e1", width=2)
        n_lines = norm.split('\n')
        if len(n_lines) == 1:
            draw.text((415, cur_y + row_h // 2), norm, fill="#334155", font=font_mid, anchor="mm")
        else:
            draw.text((415, cur_y + 32), n_lines[0], fill="#334155", font=font_mid, anchor="mm")
            draw.text((415, cur_y + 72), n_lines[1], fill="#334155", font=font_mid, anchor="mm")
        
        # Col 3 (Red BG)
        draw.rectangle([(555, cur_y), (1005, cur_y + row_h)], fill="#fef2f2", outline="#cbd5e1", width=2)
        a_lines = abn.split('\n')
        if len(a_lines) == 1:
            draw.text((780, cur_y + row_h // 2), abn, fill="#991b1b", font=font_large, anchor="mm")
        else:
            draw.text((780, cur_y + 32), a_lines[0], fill="#991b1b", font=font_large, anchor="mm")
            draw.text((780, cur_y + 72), a_lines[1], fill="#991b1b", font=font_large, anchor="mm")
        
        # Col 4
        draw.rectangle([(1005, cur_y), (1325, cur_y + row_h)], outline="#cbd5e1", width=2)
        draw.text((1165, cur_y + row_h // 2), desc, fill="#334155", font=font_small, anchor="mm")
        
        cur_y += row_h

    draw.rectangle([(25, 90), (1325, cur_y)], outline="#065f46", width=3)
    path = os.path.join(img_dir, "portal_htn_table.png")
    img.save(path, quality=95)
    print("Ultra Large Text Portal HTN Table Generated!")

make_ultra_large_portal_table()
