...
for fp in font_paths:
    if os.path.exists(fp):
        try:
            ...
            font_small = ImageFont.truetype(fp, 19)
            break
        except Exception:
            continue
...
# Ultra Large Reference Values Table
def make_ultra_large_ref_table():
    w, h = 1350, 1600
    img = Image.new("RGB", (w, h), (255, 255, 255))
    draw = ImageDraw.Draw(img)
    
    draw.rectangle([(20, 15), (1330, 75)], fill="#1e3a8a")
    draw.text((675, 45), "腹部超音波検査 臓器別 全項目 3段階臨床カットオフ判定表", fill="#ffffff", font=font_title, anchor="mm")
    ...
    cols = [
        ...
        ("要観察・境界", 600, 810),
        ("要精査・異常", 810, 1020),
        ("主な臨床的意義・アクション", 1020, 1330)
    ]
    
    y = 90
    draw.rectangle([(20, y), (1330, y + 55)], fill="#2563eb")
    ...
    for i, (organ, item, norm, bord, abnorm, desc) in enumerate(data):
        ...
        draw.rectangle([(20, cur_y), (1330, cur_y + row_h)], fill=bg_color)
        ...
        draw.rectangle([(130, cur_y), (390, cur_y + row_h)], outline="#cbd5e1", width=2)
        draw.text((260, cur_y + row_h // 2), item, fill="#0f172a", font=font_mid, anchor="mm")
        
        draw.rectangle([(390, cur_y), (600, cur_y + row_h)], fill="#f0fdf4", outline="#cbd5e1", width=2)
        draw.text((495, cur_y + row_h // 2), norm, fill="#166534", font=font_mid, anchor="mm")
        
        draw.rectangle([(600, cur_y), (810, cur_y + row_h)], fill="#fefce8", outline="#cbd5e1", width=2)
        draw.text((705, cur_y + row_h // 2), bord, fill="#854d0e", font=font_mid, anchor="mm")
        
        draw.rectangle([(810, cur_y), (1020, cur_y + row_h)], fill="#fef2f2", outline="#cbd5e1", width=2)
        draw.text((915, cur_y + row_h // 2), abnorm, fill="#991b1b", font=font_mid, anchor="mm")
        
        draw.rectangle([(1020, cur_y), (1330, cur_y + row_h)], outline="#cbd5e1", width=2)
        draw.text((1175, cur_y + row_h // 2), desc, fill="#334155", font=font_small, anchor="mm")
        
        cur_y += row_h

    draw.rectangle([(20, 90), (1330, cur_y)], outline="#1e3a8a", width=3)
    path = os.path.join(img_dir, "reference_values_table.png")
    img.save(path, quality=95)
    print("Ultra Large Reference Values Table Generated!")

make_ultra_large_ref_table()
